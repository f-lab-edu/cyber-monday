...
2. 카카오, 네이버, 구글로 가입할 수 있도록 각각의 버튼 목록이 보인다
3. 세 가지 중 하나를 선택한다
4. 소셜 로그인 인증을 마친다
...
1. 홈에서 로그인 버튼을 누른다
2. 로그인 버튼을 클릭하면 카카오, 네이버, 구글로 로그인할 수 있도록 각각의 버튼 목록이 보인다
...
1. 홈 화면으로 들어간다
2. 프로모션 & 브랜드별로 전시된 상품들이 보여진다 
3. 진열된 상품들 중 하나를 클릭하여 상세 화면으로 들어간다
4. 상세 화면에서는 해당 상품의 가격/브랜드/쿠폰 정보를 확인할 수 있다
   - 기본 가격은 정가로 표시된다
...
2. 상품 상세 화면에서 발급된 쿠폰을 다운로드한다
   - 쿠폰을 다운로드받으면 다운로드 버튼이 비활성화되며, 사용할 때까지 쿠폰을 다운로드 받을 수 없게 된다
...
1. 홈에서 보이는 브랜드 목록 중 한 곳으로 들어간다
2. 들어가면 브랜드 소개 페이지 화면과 함께 다운로드 받을 수 있도록 브랜드 전용 쿠폰이 보인다
3. 브랜드 전용 쿠폰을 다운로드 받는다
...
6. 적립금을 사용하여 총구매 가격을 낮춘다
   - 적립금은 5천 원부터 사용할 수 있다
   - 적립금은 총구매 가격의 10%까지만 사용할 수 있다
   - 10% 이상을 넘기는 금액이면 "적립금은 최대 총 금액의 10%까지만 적용 가능합니다"는 안내가 나온다
     - 이 안내가 나오면 적립금을 다시 적용해야 한다
7. 적립금은 최대 적용하기 버튼을 누르면 총구매 가격의 10%까지 할인한 금액이 계산되어 화면에 적용된다
8. 주문할 각 상품에다가 사용할 수 있는 쿠폰을 적용한다
   - 프로모션, 브랜드별 쿠폰을 종합하여 최대 1개만 선택할 수 있다
   - 다른 쿠폰을 선택해서 적용할 쿠폰을 바꿀 수 있다
9.  결제 방법들 중 하나를 선택해 결제하기 버튼을 클릭한다
   - 결제 방법에는 카드 결제, 가상 계좌, 퀵계좌이체 중 하나를 고를 수 있다
   - 결제 화면으로 넘어가기 직전에 구매한 수만큼 상품의 재고가 감소한 뒤 결제 화면으로 넘어간다
   - 결제 화면으로 넘어가기 전, 재고가 부족하다면 "재고가 부족합니다" 라는 안내가 나오면서 주문에 실패하고 주문 중 화면으로 리다이렉트된다
10. 선택한 결제 방법으로 결제를 진행한다
   - 진행 도중 결제를 안 하거나 앱을 종료하는 등으로 결제가 취소되면 주문이 취소되고, 구매하려는 개수만큼 재고가 회복된다
   - 주문이 취소되면 주문 화면으로 다시 리다이렉트 된다
11. 결제를 완료하고 결제 완료 화면으로 넘어간다
...
10. 선택한 결제 방법으로 결제를 진행한다
   - 진행 도중 결제를 안 하거나 앱을 종료하는 등으로 결제가 취소되면 주문이 취소되고, 구매하려는 개수만큼 재고가 회복된다
   - 주문이 취소되면 주문 화면으로 다시 리다이렉트 된다
11. 결제가 완료되면 결제 완료 화면으로 넘어간다
   - 결제에 사용했던 쿠폰, 적립금이 사용된다
   - 결제한 총금액의 5%만큼 적립금이 쌓인다
   - 마이 페이지 결제 내역에 해당 주문 정보와 함께 "주문완료"상태의 데이터가 쌓인다
...
4. 해당 주문 내역 상태는 "주문취소"로 변경된 것을 확인한다
   - 해당 결제 금액도 결제했던 계좌로 반환되며, 사용했던 적립금과 쿠폰도 다시 반환된다
     - 쿠폰이 반환될 시 해당 쿠폰이 이미 1개 있다면 반환 처리되지 않는다
