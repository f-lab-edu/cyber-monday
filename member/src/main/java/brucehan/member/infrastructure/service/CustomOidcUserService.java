...
import brucehan.member.middleware.handler.OAuthAttribute;
...
public class CustomOidcUserService extends UserService implements OAuth2UserService<OidcUserRequest, OidcUser> {
    ...
    public OidcUser loadUser(OidcUserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        log.info("member - roles check: {}", member.getRole());
        log.info("member - picture: {}", member.getProfileImage());
        log.info("oidcuser - image {}", oidcUser.getAttributes().get("picture"));
        member.updateProfileImageIfAbsent((String) oidcUser.getAttributes().get("picture"));
    ...
    private OidcUser createOAuth2User(MemberEntity memberEntity, OidcUserRequest userRequest) {
        Collection<? extends GrantedAuthority> authorities = memberEntity.initSimpleGrantedAuthorities();
